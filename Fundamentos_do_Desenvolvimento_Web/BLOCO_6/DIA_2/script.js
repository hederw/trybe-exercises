...
function createStateOptions() {
    ...
    for (let index of stateOptions) {
      let createOptions = document.createElement('option');
      states.appendChild(createOptions).innerText = index;
      states.appendChild(createOptions).value = index;
    }
  }
  
  //Objeto para verificação dos campos
  let inputs = {
    name: {
      maxLength: 40,
      required: true,
  ...
  function defaultValidation(input, name){
    let trimmed = input.value.trim();//trim retira todos os espacos em branco entre palavras
    let validation = inputs[name];//recebe 
